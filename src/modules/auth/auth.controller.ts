...
export class AuthController {
  ...
  @Post('login')
  @ApiOperation({
    summary: 'Attendees Login Endpoint',
  })
  @ApiResponse({
  ...
  @Post('registration')
  @ApiOperation({
    summary: 'Attendees Registration Endpoint',
  })
  @ApiResponse({
    description: 'Something went wrong',
    status: HttpStatus.BAD_REQUEST,
  ...
  async registration(@Body() registrationDto: RegistrationDto, @Request() req) {
    return {
  ...
  async verifyEmail(@Body() emailVerificationDto: EmailVerificationDto) {
    return {
      ...
      result: await this.authService.emailVerification(emailVerificationDto),
    };
  }
  ...
  //     status: HttpStatus.CREATED,
  //     message: 'Code sent',
  //     result: await this.authService.forgetPassword(verificationCodeSenderDto),
  //   };
  // }
  ...
  private async codeVerification(@Body() forgetPasswordDto: ForgetPasswordDto) {
    return {
      status: HttpStatus.OK,
  ...
  private async recoverPassword(@Body() updatePasswordDto: UpdatePasswordDto) {
    return {
      status: HttpStatus.CREATED,
